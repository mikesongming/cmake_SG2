...
geocentric_data::geocentric_data(date const & ut) noexcept: ut{ut}
{
    if (ut.isnat()) {
        tt = nat;
    } else {
        ...
        int64_t delta_tt = static_cast<int64_t>(approx_deltat_msc.compute(year));
        if (delta_tt == std::numeric_limits<int16_t>::min()) {
            tt.msec = std::numeric_limits<int16_t>::min();
        } else {
